...
#define MQTT_PAYLOAD_PUB                    "{\"services\":[{\"service_id\":\"attribute\",\"properties\":{\"Dimmable_LED\":%d,\"infrared\":%.2f,\"DIMHost\":\"%s\"}}]}"   // 设备信息
...
{
  // 代表具体的数值
  uint16_t Dimmable_LED;  // 可调节亮度的LED灯的数值
  float infrared;  // 人体红外传感器的数值
  // 代表字符串
  char DIMHost[50]; // 可调节亮度的LED灯的IP地址
}msg_sensorData_t;
...
void mqtt_send_task(void *argument)
{
  ...
  {
    // 更新数值
    sensorData.infrared = global_inte;
    sensorData.Dimmable_LED = light_value;
    strcpy(sensorData.DIMHost, DIM_HOST);
    // 打印结果
    console_log_info("Dimmable_LED:%d infrared:%.2f", 
      sensorData.Dimmable_LED,
      sensorData.infrared
    );
    console_log_info("DIMHost:%s", 
      sensorData.DIMHost
    );
    // 上传华为云
...
int8_t mqttClient_sub_callback(unsigned char *topic, unsigned char *payload)
{
  ...
  else 
  {
    ...
    if (root)// 解析JSON数据
    {
      ...
      if (command_name)
      {
        if (!strcmp(command_name->valuestring, "Dimmable_LED"))
        {
