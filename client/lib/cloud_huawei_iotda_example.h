...
#define MQTT_PAYLOAD_PUB                    "{\"services\":[{\"service_id\":\"attribute\",\"properties\":{\"temperature\":%.2f,\"humidity\":%.2f,\"ClientHost\":\"%s\"}}]}"   // 设备信息
...
void mqtt_send_task(void *argument)
{
  ...
  {
    // 更新数值
    sensorData.temperature = global_temp;
    sensorData.humidity = global_humi;
    strcpy(sensorData.ClientHost, CLIENT_HOST);
    // 打印结果
    console_log_info("buzzer:%d temperature:%.2f humidity:%.2f", 
      sensorData.temperature, 
      sensorData.humidity
    );
    console_log_info("ClientHost:%s", 
      sensorData.ClientHost
    );
    // 上传华为云
...
int8_t mqttClient_sub_callback(unsigned char *topic, unsigned char *payload)
{
  ...
  else 
  {
    ...
    if (root)// 解析JSON数据
    {
      ...
      if (command_name)
      {
        if (!strcmp(command_name->valuestring, "RGBHost"))
        {
          value = cJSON_GetObjectItem(paras, "value");
          strcpy(RGB_HOST,value->valuestring);
          ret_code = 0;   // 0为成功
        }
        else if (!strcmp(command_name->valuestring, "DIMHost"))
        {
          value = cJSON_GetObjectItem(paras, "value");
          strcpy(DIM_HOST,value->valuestring);
          ret_code = 0;   // 0为成功
        }
        else if (!strcmp(command_name->valuestring, "DoorHost"))
        {
          value = cJSON_GetObjectItem(paras, "value");
          strcpy(DOOR_HOST,value->valuestring);
          ret_code = 0;   // 0为成功
        }
      }
